...
`node/transaction.sh` 为主配置重建、节点创建/替换及手动 Reality 密钥更新提供统一提交入口：

- 在目标文件所在文件系统建立权限受限的临时目录，复制主配置和其他节点组成完整候选配置集。
- 先进行 JSON 检查和真实核心 `check -c ... -C ...` 校验，再以 `mv` 替换目标；更名时先写新文件、后删除旧文件。
- 使用共享写入锁避免这些入口并发提交；拒绝路径越界、目标符号链接和意外覆盖已有节点。
- 已处理的替换/删除失败会恢复原文件；恢复失败保留临时目录并输出恢复位置。快照复制失败会阻止继续修改。
- 调用方只在配置提交成功后执行节点关联服务操作；首次创建主配置时延后重启，等待节点提交。

这是**配置文件级保护**，不是整个系统的事务：未统一接管所有历史写入入口，Caddy、DNS 等旧写入流程仍需后续迁移；不自动回滚服务、证书、防火墙或下载产生的状态，也不提供断电、SIGKILL 后的自动恢复。异常终止留下写锁时，先确认没有正在运行的写入命令并检查恢复目录，不要直接盲目重试。多个文件的更名提交不是一次目录级原子交换。

`dry-run` 下主配置/Caddy 重建、Caddy 安装、自动端口防火墙放行、SS2022 临时配置探测及自动 SNI 存储/健康缓存/最近使用记录均不写入。尚无经过检查的预演路径的命令会明确提示未执行，不透传给核心；预演仍可能进行只读网络探测。

生产配置测试对生成、断言和读取逐项显式检查退出状态，不能依赖条件子 Shell 中的 `set -e`。`test-test-failure-detection.sh` 用故障注入保证错误确实会使测试失败；设置 `SING_BOX_CORE_BIN` 时也验证真实核心发布测试的失败检测。
